...
document.addEventListener('DOMContentLoaded', function () {
  ...
  var collapseTimer;
  var headings = entries.map(function (link) { return document.getElementById(decodeURIComponent(link.getAttribute('href').slice(1))); }).filter(Boolean);
  ...
  if (window.IntersectionObserver) {
    var observer = new IntersectionObserver(function (records) {
      records.forEach(function (record) { if (record.isIntersecting) setActive(headings.indexOf(record.target)); });
